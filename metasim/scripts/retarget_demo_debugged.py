...
def build_state_entry(src_state_t, src_robot_name, tgt_robot_name, tgt_joint_names, tgt_q_list, base_pos, base_rot):
    # copy all non-robot objects (e.g., box_base, table, etc.)
    env_state = {k: v for k, v in (src_state_t or {}).items() if k != src_robot_name}
    # inject target robot state
    env_state[tgt_robot_name] = {
        "pos": base_pos,
        "rot": base_rot,
        "dof_pos": dict(zip(tgt_joint_names, tgt_q_list)),
    }
    return env_state
...
def single(src_path, tgt_path, src_robot, tgt_robot):
    ...
    for i_succ in tqdm(torch.where(succ)[0], desc=f"Saving trajectories for {tgt_robot_cfg.name}"):
        ep_len = int(episode_lengths[i_succ].item())
        tgt_base_pos = robot_pos[i_succ].tolist()  # override if Sawyer mount differs
        tgt_base_rot = robot_quat[i_succ].tolist()

        demo = {"actions": [], "states": [], "init_state": {}}
        src_states = src_demos[i_succ].get("states", None)

        for t in range(ep_len):
            q_list = tgt_robot_qpos[i_succ, t].tolist()

            # actions -> use TARGET joint names (Sawyer), not panda_*
            demo["actions"].append({
                "dof_pos_target": dict(zip(tgt_robot_joint_names, q_list))
            })

            # states -> copy env, swap robot entry
            src_state_t = src_states[t] if (src_states is not None and t < len(src_states)) else {}
            demo["states"].append(
                build_state_entry(
                    src_state_t,
                    src_robot_cfg.name,
                    tgt_robot_cfg.name,
                    tgt_robot_joint_names,
                    q_list,
                    tgt_base_pos,
                    tgt_base_rot,
                )
        ...
        # init_state: deep copy then swap robot entry
        init_state = copy.deepcopy(src_demos[i_succ]["init_state"])
        init_state.pop(src_robot_cfg.name, None)
        init_state[tgt_robot_cfg.name] = {
            "dof_pos": dict(zip(tgt_robot_joint_names, tgt_robot_qpos[i_succ, 0].tolist())),
            "pos": tgt_base_pos,
            "rot": tgt_base_rot,
        }
        demo["init_state"] = init_state

        # sanity
        assert len(demo["actions"]) == len(demo["states"])
        assert tgt_robot_cfg.name in demo["states"][0]
